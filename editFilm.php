...
if (isset($_POST['title'])  && isset($_POST['description']) && isset($_POST['release_year'])) {
  ...
  $sql = 'UPDATE actor SET title=:title, description=:description, release_year=:release_year WHERE film_id=:film_id';
  $statement = $connection->prepare($sql);
  if ($statement->execute([':title' => $title, ':description' => $description,':release_year' => $release_year, ':film_id' => $film_id])) {
